...
def min_pairwise_distance_coreset(X, y, target_size, random_state=12227):
    rng = check_random_state(random_state)
    num_samples, num_features = X.shape

    # Select a random point as the initial point in the coreset
    coreset_indices = [rng.choice(num_samples)]
    
    while len(coreset_indices) < target_size:
        # Compute pair-wise distances between points in the coreset and all points in the dataset
        distances = pairwise_distances(X[coreset_indices], X, metric='euclidean')

        # Find the point farthest away from the coreset
        max_distances = np.max(distances, axis=0)

        # Find the index of the point with the maximum distance
        new_point_index = np.argmax(max_distances)

        # Add the new point to the coreset
        coreset_indices.append(new_point_index)
...
def reconstruct_coreset(X_train, y_train, coreset_size):
    ...
    # Create the new X_train and y_train from the coreset indices
    new_X_train = X_train[coreset_indices]
    new_y_train = y_train[coreset_indices]
...
if __name__ == '__main__':
    ...
    X_train = np.random.rand(3771, 6)
    y_train = np.random.randint(0, 7, 3771) 

    # Create a coreset of size 1024
    coreset_size = 1024
    
    X, y = reconstruct_coreset(X_train, y_train, coreset_size)
    
    print(X.shape, y.shape)
    print(X)
    print(y)
    #coreset_X, coreset_y = min_pairwise_distance_coreset(X_train, y_train, coreset_size)

    #print(cs_construction_min_pairwise(X_train, y_train, np.unique(y_train)*[int(coreset_size/np.unique(y_train))]))

    #print(np.array(X_train).shape, np.array(X_train))

    #coreset = cs_construction_min_pairwise(np.array(X_train), np.array(y_train), [2])
    
    #print(coreset)

    #X = np.hstack((X_train, y_train.reshape(-1, 1)))
    #print(X)
    #print(dm_sum_pairwise(euclidean_distances(X_train), 5))

    # Now, 'coreset_X' contains the selected subset of the training data, and 'coreset_y' contains the corresponding labels
    #print("Coreset X shape:", coreset_X.shape)
    #print("Coreset y shape:", coreset_y.shape)
    exit(0)
